...
local _kwList = {'and', 'break', 'do', 'else', 'elseif', 'end', 'false', 'for', 'function', 'if', 'in', 'local', 'nil', 'not', 'or', 'repeat', 'return', 'then', 'true', 'until', 'while'}
        ...
        if love.keyboard.isDown('lctrl') then
            ...
            elseif a:lower() == 'c' then
                ...
                if not _Debug.trackKeys[a] then
                    _Debug.trackKeys[a] = {
                        time = _Debug.keyRepeatInterval - _Debug.keyRepeatDelay
                    }
                end
            end
        else
            ...
            if _Debug.drawOverlay then
                for key, d in pairs(_Debug.trackKeys) do
                    ...
                    if type(key) == 'string' and key ~= " " then
                        if not (_key == "{" or _key == "}") and love.keyboard.isDown(key:lower()) then
                            d.time = d.time + dt
